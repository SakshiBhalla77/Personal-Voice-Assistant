...
pyttsx3.speak("Greetings, this is strydo, a personal voice assistant made by Sakshi, which will be there for you everytime you need it. Tell me, you wanna talk to Ben or Kate?")
...
while True:
    ...
    if ((("run" in p) or ("execute" in p) or ("use" in p) or ("open" in p) or ("start" in p) or ("launch" in p)) and ("chrome" in p or "browser" in p)):
        ...
        subprocess.run("start microsoft.windows.camera:", shell=True)
            ...
            pyttsx3.speak("Thanks to Sakshi. Further It's a secret")
    elif 'joke' in p:
